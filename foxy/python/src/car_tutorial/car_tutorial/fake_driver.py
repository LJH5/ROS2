...
class FakeDriver(Node):
    def __init__(self):
        ...
        self.pub_vel_raw = self.create_publisher(TwistStamped, "vel_raw", qos_profile)
        self.pub_img_raw = self.create_publisher(Image, "img_raw", 10)

        # subscriber
        self.sub_cmd_vel = self.create_subscription(Twist, "cmd_vel", self.cmd_vel_callback, qos_profile)
